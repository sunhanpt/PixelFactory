...
static void PHYSFS_utf8ToUcs4(const char *src, uint32 *dst, uint64 len)
{
	len -= sizeof(uint32);   /* save room for null char. */
...
static void PHYSFS_utf8ToUcs2(const char *src, uint16 *dst, uint64 len)
{
...
while (len) \
{ \
const uint32 cp = (uint32) ((typ) (*(src++))); \
if (cp == 0) break; \
utf8fromcodepoint(cp, &dst, &len); \
} \
*dst = '\0'; \

static void PHYSFS_utf8FromUcs4(const uint32 *src, char *dst, uint64 len)
{
	UTF8FROMTYPE(uint32, src, dst, len);
} /* PHYSFS_utf8FromUcs4 */

static void PHYSFS_utf8FromUcs2(const uint16 *src, char *dst, uint64 len)
{
...
{
#ifdef _WIN32
	PHYSFS_utf8ToUcs2(in, (uint16 *)out, len);
#else
	PHYSFS_utf8ToUcs4(in, (uint32 *)out, len);
#endif
}
...
{
#ifdef _WIN32
	PHYSFS_utf8FromUcs2((const uint16 *)in, out, len);
#else
	PHYSFS_utf8FromUcs4((const uint32 *)in, out, len);
#endif
}
