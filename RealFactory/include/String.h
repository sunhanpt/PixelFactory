...
class FString;
static size_t MultibyteToWString(FString<wchar_t>& destination, const char* source, uint32 sourceSize);
...
static eLocaleID locale_current = eLocaleID::RF_LOCALE_ANSI;
...
public:
	...
	//! Constructor
	FString(const FString<T, TAlloc>& other)
	...
	FString(const FString<B, A>& other)
		: array(0), allocated(0), used(0)
	{
		*this = other;
	}
	...
	//! Assignment operator
	FString<T, TAlloc>& operator=(const FString<T, TAlloc>& other)
	{
		if (this == &other)
			return *this;
	...
	template <class B, class A>
	FString<T, TAlloc>& operator=(const FString<B, A>& other)
	{
		*this = other.C_Str();
		return *this;
	}
	...
	template <class B>
	FString<T, TAlloc>& operator=(const B* const c)
	{
		if (!c)
		{
			if (!array)
	...
	FString<T, TAlloc> operator+(const FString<T, TAlloc>& other) const
	{
		FString<T, TAlloc> str(*this);
		str.Append(other);
	...
	template <class B>
	FString<T, TAlloc> operator+(const B* const c) const
	{
	...
	//! Equality operator
	bool operator==(const FString<T, TAlloc>& other) const
	{
	...
	bool Equals_ignore_case(const FString<T, TAlloc>& other) const
	{
	...
	\return True if the strings are equal ignoring case. */
	bool Equals_substring_ignore_case(const FString<T, TAlloc>&other, const int32 sourcePos = 0) const
	{
	...
	bool Lower_ignore_case(const FString<T, TAlloc>& other) const
	{
		for (uint32 i = 0; array[i] && other.array[i]; ++i)
		{
	...
	bool Equalsn(const FString<T, TAlloc>& other, uint32 n) const
	{
		uint32 i;
	...
	FString<T, TAlloc>& Append(T character)
	{
		if (used + 1 > allocated)
			Reallocate(used + 1);
	...
	FString<T, TAlloc>& Append(const T* const other, uint32 length = 0xffffffff)
	{
		if (!other)
			return *this;
	...
	FString<T, TAlloc>& Append(const FString<T, TAlloc>& other)
	{
		if (other.Size() == 0)
			return *this;
	...
	or -1 if not found. */
	uint32 FindFirstChar(const T* const c, uint32 count = 1) const
	{
		if (!c || !count)
			return -1;
	...
	FString<T> SubString(uint32 begin, int32 length, bool make_lower = false) const
	{
	...
	FString<T, TAlloc>& operator += (const T* const c)
	{
		Append(c);
		return *this;
	}
	...
	FString<T, TAlloc>& operator += (const int i)
	{
	...
	//! Appends a string representation of a number to this string
	/** \param i Number to append. */
	FString<T, TAlloc>& operator += (const unsigned int i)
	{
		Append(string<T, TAlloc>(i));
		return *this;
	}
	...
	//! Appends a string representation of a number to this string
	/** \param i Number to append. */
	FString<T, TAlloc>& operator += (const long i)
	{
		Append(string<T, TAlloc>(i));
		return *this;
	}
	...
	//! Appends a string representation of a number to this string
	/** \param i Number to append. */
	FString<T, TAlloc>& operator += (const unsigned long i)
	{
		Append(FString<T, TAlloc>(i));
		return *this;
	}
	...
	//! Appends a string representation of a number to this string
	/** \param i Number to append. */
	FString<T, TAlloc>& operator += (const double i)
	{
		Append(FString<T, TAlloc>(i));
		return *this;
	}
	...
	//! Appends a string representation of a number to this string
	/** \param i Number to append. */
	FString<T, TAlloc>& operator += (const float i)
	{
		Append(FString<T, TAlloc>(i));
		return *this;
	}
	...
	FString<T, TAlloc>& Replace(T toReplace, T replaceWith)
	{
		for (uint32 i = 0; i<used - 1; ++i)
	...
	FString<T, TAlloc>& Replace(const FString<T, TAlloc>& toReplace, const FString<T, TAlloc>& replaceWith)
	{
		if (toReplace.Size() == 0)
			return *this;
	...
	FString<T, TAlloc>& Remove(const FString<T, TAlloc>& toRemove)
	{
	...
	FString<T, TAlloc>& RemoveChars(const FString<T, TAlloc> & characters)
	{
	...
	FString<T, TAlloc>& Trim(const FString<T, TAlloc> & whitespace = " \t\n\r")
	{
	...
	FString<T, TAlloc>& Erase(uint32 index)
	{
		_RF_DEBUG_BREAK_IF(index >= used) // access violation
	...
	*/
	...
	uint32 Split(container& ret, const T* const delimiter, uint32 countDelimiters = 1, bool ignoreEmptyTokens = true, bool keepSeparators = false) const
	{
		if (!delimiter)
			return 0;
		...
		uint32 tokenStartIdx = 0;
		for (uint32 i = 0; i<used; ++i)
		{
			for (uint32 j = 0; j<countDelimiters; ++j)
			{
				if (array[i] == delimiter[j])
				{
					if (keepSeparators)
					{
						ret.push_back(FString<T, TAlloc>(&array[tokenStartIdx], i + 1 - tokenStartIdx));
					}
					else
					{
						if (i - tokenStartIdx > 0)
							ret.push_back(FString<T, TAlloc>(&array[tokenStartIdx], i - tokenStartIdx));
						else if (!ignoreEmptyTokens)
							ret.push_back(FString<T, TAlloc>());
					}
					tokenStartIdx = i + 1;
					break;
				}
		...
		if ((used - 1) > tokenStartIdx)
			ret.push_back(FString<T, TAlloc>(&array[tokenStartIdx], (used - 1) - tokenStartIdx));

		return ret.size() - oldSize;
	}

	friend size_t MultibyteToWString(FString<wchar_t>& destination, const char* source, uint32 sourceSize);
...
#pragma warning(pop)
		...
		if (written != (size_t)-1)
		{
			destination.used = (uint32)written + 1;
